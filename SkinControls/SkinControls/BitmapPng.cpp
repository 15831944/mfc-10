...
BOOL CBitmapPng::CCallback::PngLoadImage(CFile* pFile, png_byte **ppbImageData,
	png_uint_32 *piWidth, png_uint_32 *piHeight, int *piChannels, png_color *pBkgColor)
{
	png_byte            pbSig[8];
	int                 iBitDepth;
...
HBITMAP CBitmapPng::ConvertToBitmap(BYTE* pbImage, CSize szImage,  int cImgChannels) const
{
	LONG xImg, yImg;
	BYTE *src, *dst;
	BYTE r, g, b, a;
...
BOOL CBitmapPng::LoadFromFile(LPCTSTR lpszFileName)
{
	m_bAlpha = FALSE;
	DeleteObject();
...
BOOL CBitmapPng::LoadFromResource(HMODULE hModule, HRSRC hRes)
{
	m_bAlpha = FALSE;
	DeleteObject();
...
BOOL CBitmapPng::LoadFromMemory(BYTE* pBuffer, DWORD dwSize)
{
	...
	CMemFile file(pBuffer, dwSize);

	return LoadFromFile(&file);
}

BOOL CBitmapPng::LoadFromFile(CFile* pFile)
{
	m_bAlpha = FALSE;
	DeleteObject();
