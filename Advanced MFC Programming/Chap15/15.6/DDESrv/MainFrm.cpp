...
TCHAR CMainFrame::m_szPokeItem[]=TEXT("Poke");
...
HSZ CMainFrame::m_hszPokeItem=0;
...
)
{
	switch(wType)
	{
		...
		case XTYP_POKE:
		{
			...
			(
				!::DdeCmpStringHandles(hszTopic, m_hszTopicName) && 
				!::DdeCmpStringHandles(hszItemName, m_hszPokeItem) &&
				wFormat == CF_TEXT &&
				hConv == m_hConvServer
			)
			{
		...
		case XTYP_EXECUTE:
		{
			...
			)
			{
				...
				lpByte=new BYTE[nSize];
				::DdeGetData(hData, lpByte, nSize, 0);
...
void CMainFrame::Hszize()
{
	...
	Printf("Supported item name: %s\r\n", m_szTextItem);
	m_hszPokeItem=::DdeCreateStringHandle(m_dwInst, m_szPokeItem, NULL);
	Printf("Supported item name: %s\r\n", m_szPokeItem);
}
...
void CMainFrame::UnHszize()
{
	...
	::DdeFreeStringHandle(m_dwInst, m_hszPokeItem);
}
