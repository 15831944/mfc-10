...
{
	delete pInfo->m_pPD;
	pInfo->m_pPD=new CPrnDlg(FALSE);
	pInfo->m_pPD->m_pd.nMinPage=1;
	pInfo->m_pPD->m_pd.nMaxPage=0xFFFF;
	pInfo->m_pPD->m_pd.hInstance=AfxGetInstanceHandle();
	pInfo->m_pPD->m_pd.lpPrintTemplateName=MAKEINTRESOURCE(PRINTDLG);
	pInfo->m_pPD->m_pd.Flags|=PD_ENABLEPRINTTEMPLATE;
	if(DoPreparePrinting(pInfo) == FALSE)return FALSE;
	pInfo->SetMaxPage(0xFFFFFFFE);
	return TRUE;
}
