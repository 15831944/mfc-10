...
public:
	...
	void ResetTracker();
	void GetCurrentBitmapInfoHeader(LPBITMAPINFOHEADER);
	void FillSelectionWithBgdColor();
	void PasteDIB(HGLOBAL);
	void ChangeTrackerRectRatio(int);
	LPVOID GetLPBits(){return &m_lpBits;}
	CPoint NormalizePtPosition(CPoint);
	CRect NormalizeTrackerRect(CRect);
	CRect UnnormalizeTrackerRect(CRect);
	CRect GetNormalizedTrackerRect(){return NormalizeTrackerRect(m_trackerSel.m_rect);}
	BOOL MouseWithinBitmap();
	BOOL IsTrackerValid(){return !m_trackerSel.m_rect.IsRectEmpty();}
