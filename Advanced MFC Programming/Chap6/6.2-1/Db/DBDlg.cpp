...
{
	CPropertySheet/*CDialog*/::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDBDlg)
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CDBDlg, CPropertySheet)//CDialog)
	//{{AFX_MSG_MAP(CDBDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
{
	CPropertySheet/*CDialog*/::OnInitDialog();
...
void CDBDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	{
		CPropertySheet/*CDialog*/::OnSysCommand(nID, lParam);
	}
}
...
void CDBDlg::OnPaint() 
{
	...
	{
		CPropertySheet/*CDialog*/::OnPaint();
	}
}
