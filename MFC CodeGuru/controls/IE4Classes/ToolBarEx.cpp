...
static const TB_COLORMAP SysColorMap[] =
{
...
static HBITMAP LoadSysColorBitmap(HINSTANCE hInst, HRSRC hRsrc, BOOL bMono)
{
  HGLOBAL hglb;
  ...
  for (int iColor = 0; iColor < nColorTableSize; iColor++)
  {
    ...
    for (int i = 0; i < sizeof(SysColorMap)/sizeof(SysColorMap[0]); i++)
    {
...
static DWORD GetComCtlVersion()
{
  ...
  pfn = (DLLGETVERSIONPROC)GetProcAddress(hInst, "DllGetVersion");
...
BOOL CToolBarEx::OnNcCreate(LPCREATESTRUCT lpCreateStruct)
{
  if (!CControlBarEx::OnNcCreate(lpCreateStruct))
    return FALSE;
...
void CToolBarEx::SetOwner(CWnd* pOwnerWnd)
{
  ASSERT_VALID(this);
  if (m_hWnd != NULL)
  {
  ...
  CControlBarEx::SetOwner(pOwnerWnd);
}

void CToolBarEx::SetSizes(SIZE sizeButton, SIZE sizeImage)
{
  ASSERT_VALID(this);
...
void CToolBarEx::SetHeight(int cyHeight)
{
  ASSERT_VALID(this);
  ...
  if (m_cyTopBorder < 0)
  {
    TRACE1("Warning: CToolBarEx::SetHeight(%d) is smaller than button.\n",
      nHeight);
...
BOOL CToolBarEx::LoadToolBar(LPCTSTR lpszResourceName)
{
  ...
  CToolBarExData* pData = (CToolBarExData*)LockResource(hGlobal);
  if (pData == NULL)
    return FALSE;
...
BOOL CToolBarEx::AddReplaceBitmap(HBITMAP hbmImageWell)
{
...
BOOL CToolBarEx::SetButtons(const UINT* lpIDArray, int nIDCount)
{
  ASSERT_VALID(this);
  ...
  if (lpIDArray != NULL)
  {
    ...
    for (int i = 0; i < nIDCount; i++)
    {
      ...
      if ((button.idCommand = *lpIDArray++) == 0)
      {
        ...
        ASSERT(ComCtlVersion != -1);
        if ((GetStyle() & TBSTYLE_FLAT) || ComCtlVersion == VERSION_IE4)
          button.iBitmap = 6;
        else
          button.iBitmap = 8;
      }
...
int CToolBarEx::CommandToIndex(UINT nIDFind) const
{
  ASSERT_VALID(this);
  ASSERT(::IsWindow(m_hWnd));
...
UINT CToolBarEx::GetItemID(int nIndex) const
{
  ASSERT_VALID(this);
  ASSERT(::IsWindow(m_hWnd));
...
void CToolBarEx::GetItemRect(int nIndex, LPRECT lpRect) const
{
  ASSERT_VALID(this);
  ASSERT(::IsWindow(m_hWnd));
...
void CToolBarEx::Layout()
{
  ...
  if ((m_dwStyle & CBRS_FLOATING) && (m_dwStyle & CBRS_SIZE_DYNAMIC))
    ((CToolBarEx*)this)->CalcDynamicLayout(0, LM_HORZ | LM_MRUWIDTH | LM_COMMIT);
  else if (bHorz)
    ((CToolBarEx*)this)->CalcDynamicLayout(0, LM_HORZ | LM_HORZDOCK | LM_COMMIT);
  else
    ((CToolBarEx*)this)->CalcDynamicLayout(0, LM_VERTDOCK | LM_COMMIT);
}

UINT CToolBarEx::GetButtonStyle(int nIndex) const
{
  ASSERT_VALID(this);
  ASSERT(::IsWindow(m_hWnd));
...
void CToolBarEx::SetButtonStyle(int nIndex, UINT nStyle)
{
  ASSERT_VALID(this);
  ASSERT(::IsWindow(m_hWnd));
...
CSize CToolBarEx::CalcSize(TBBUTTON* pData, int nCount)
{
...
int CToolBarEx::WrapToolBar(TBBUTTON* pData, int nCount, int nWidth)
{
  ASSERT(pData != NULL && nCount > 0);
...
void CToolBarEx::SizeToolBar(TBBUTTON* pData, int nCount, int nLength, BOOL bVert)
{
  ASSERT(pData != NULL && nCount > 0);
...
CSize CToolBarEx::CalcLayout(DWORD dwMode, int nLength)
{
  ...
  if (nCount > 0)
  {
    ...
    if (dwMode & LM_COMMIT)
    {
      ...
      if (nControlCount > 0)
      {
        pControl = new TB_CONTROLPOS[nControlCount];
        nControlCount = 0;
  ...
  //BLOCK: Adjust Margins
  {
    ...
    CSize size = CControlBarEx::CalcFixedLayout((dwMode & LM_STRETCH), (dwMode & LM_HORZ));
...
void CToolBarEx::GetButtonInfo(int nIndex, UINT& nID, UINT& nStyle, int& iImage) const
{
  ASSERT_VALID(this);
  ASSERT(::IsWindow(m_hWnd));
...
void CToolBarEx::SetButtonInfo(int nIndex, UINT nID, UINT nStyle, int iImage)
{
  ASSERT_VALID(this);
...
int CToolBarEx::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
  ASSERT_VALID(this);
  ASSERT(::IsWindow(m_hWnd));

  // check child windows first by calling CControlBarEx
  int nHit = CControlBarEx::OnToolHitTest(point, pTI);
  if (nHit != -1)
    return nHit;
  ...
  for (int i = 0; i < nButtons; i++)
  {
    ...
    if (pBar->DefWindowProc(TB_GETITEMRECT, i, (LPARAM)&rect))
    {
        ...
        !(button.fsStyle & TBSTYLE_SEP))
      {
        ...
        if (pTI != NULL && pTI->cbSize >= sizeof(OLDTOOLINFO))
        {
          pTI->hwnd = m_hWnd;
          pTI->rect = rect;
          pTI->uId = nHit;
...
BOOL CToolBarEx::SetButtonText(int nIndex, LPCTSTR lpszText)
{
...
CString CToolBarEx::GetButtonText(int nIndex) const
{
  CString strResult;
...
void CToolBarEx::GetButtonText(int nIndex, CString& rString) const
{
  if (m_pStringMap != NULL)
  {
...
BEGIN_MESSAGE_MAP(CToolBarEx, CControlBarEx)
...
BOOL CToolBarEx::OnEraseBkgnd(CDC*)
{
  return (BOOL)Default();
}

UINT CToolBarEx::OnNcHitTest(CPoint)
{
  return HTCLIENT;
}

void CToolBarEx::OnNcCalcSize(BOOL /*bCalcValidRects*/, NCCALCSIZE_PARAMS* lpncsp)
{
  // calculate border space (will add to top/bottom, subtract from right/bottom)
  CRect rect; rect.SetRectEmpty();
  BOOL bHorz = (m_dwStyle & CBRS_ORIENT_HORZ) != 0;
  CControlBarEx::CalcInsideRect(rect, bHorz);
  ASSERT(ComCtlVersion != -1);
  ASSERT(ComCtlVersion >= VERSION_IE4 || rect.top >= 2);

  // adjust non-client area for border space
  lpncsp->rgrc[0].left += rect.left;
  lpncsp->rgrc[0].top += rect.top;
  // previous versions of COMCTL32.DLL had a built-in 2 pixel border
  if (ComCtlVersion < VERSION_IE4)
    lpncsp->rgrc[0].top -= 2;
...
void CToolBarEx::OnWindowPosChanging(LPWINDOWPOS lpWndPos)
{
  // not necessary to invalidate the borders
  DWORD dwStyle = m_dwStyle;
  m_dwStyle &= ~(CBRS_BORDER_ANY);
  CControlBarEx::OnWindowPosChanging(lpWndPos);
  m_dwStyle = dwStyle;
...
LRESULT CToolBarEx::OnSetButtonSize(WPARAM, LPARAM lParam)
{
  return OnSetSizeHelper(m_sizeButton, lParam);
}

LRESULT CToolBarEx::OnSetBitmapSize(WPARAM, LPARAM lParam)
{
  return OnSetSizeHelper(m_sizeImage, lParam);
}

LRESULT CToolBarEx::OnSetSizeHelper(CSize& size, LPARAM lParam)
{
...
/////////////////////////////////////////////////////////////////////////////
// CToolBarEx idle update through CToolExCmdUI class

class CToolExCmdUI : public CCmdUI        // class private to this file !
{
...
void CToolExCmdUI::Enable(BOOL bOn)
{
  m_bEnableChanged = TRUE;
  CToolBarEx* pToolBar = (CToolBarEx*)m_pOther;
  ASSERT(pToolBar != NULL);
  ASSERT_KINDOF(CToolBarEx, pToolBar);
...
void CToolExCmdUI::SetCheck(int nCheck)
{
...
void CToolExCmdUI::SetText(LPCTSTR)
{
  // ignore it
}
...
/////////////////////////////////////////////////////////////////////////////
// CToolBarEx diagnostics

#ifdef _DEBUG
void CToolBarEx::AssertValid() const
{
  // Note: CControlBarEx::AssertValid is not called because it checks for
  //  m_nCount and m_pData to be in sync, which they are not in CToolBarEx.
...
void CToolBarEx::Dump(CDumpContext& dc) const
{
  CControlBarEx::Dump(dc);
...
IMPLEMENT_DYNAMIC(CToolBarEx, CControlBarEx)
