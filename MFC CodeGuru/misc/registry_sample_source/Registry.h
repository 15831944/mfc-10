...
// last revised $Date: 7.05.98 22:05 $ $Revision: 2 $
...
class CRegistry {
	...
	private:
		...
		// itself recursivly.
		BOOL			RegTreeWalk( LPCTSTR SubKey, HKEY hSubKey, UINT Flags ) ;
#endif	// _REG_NO_TREEWALK

		// check the given keyname whether it is valid or not. Puts a warning
		// to the debugger's output window, if it detects problems.
		static void			VerifyKeyName(LPCTSTR);
};
