...
CHyperLink::CHyperLink()
{
	...
	// Set default styles
	m_dwStyle = HLS_UNDERLINE | HLS_AUTOSIZE | HLS_GETFOCUSONCLICK;
}
...
{		    
	...
	if (pFont != NULL) {
		LOGFONT lf;
		pFont->GetLogFont(&lf);
		lf.lfUnderline = BITSET(m_dwStyle, HLS_UNDERLINE);
		...
		// auto-resizing off
		CLEARBITS(m_dwStyle, HLS_AUTOSIZE);
	}
	
	if (!BITSET(m_dwStyle,HLS_NOHANDCURSOR))
...
{		
	...
	if (m_bOverControl && BITSET(m_dwStyle, HLS_USEHOVER))
		pDC->SetTextColor(g_crHoverColor);
	else if (!BITSET(m_dwStyle,HLS_NOACTIVECOLOR) && m_bLinkActive)
...
{
	if (BITSET(m_dwStyle, HLS_GETFOCUSONCLICK))
		SetFocus();				// Set the focus and make the link active
	if (BITSET(m_dwStyle,HLS_DOWNCLICK))
		FollowLink();
	m_bLinkActive = TRUE;
}
...
{
	if (m_bLinkActive && !BITSET(m_dwStyle,HLS_DOWNCLICK))
		FollowLink();
}
								  ...
								  BOOL bApply /* =TRUE */)
{
	...
	if (bApply && ::IsWindow(GetSafeHwnd())) {
		...
		if (BITSET(dwAdd,HLS_UNDERLINE) || BITSET(dwRemove,HLS_UNDERLINE))
			SwitchUnderline();		
		if (BITSET(dwAdd,HLS_AUTOSIZE))
			AdjustWindow();		
		if (BITSET(dwRemove, HLS_USEHOVER))
			Invalidate();
	}
	return TRUE;
}
...
{	
	...
	if (!BITSET(m_dwStyle,HLS_AUTOSIZE)) 
        return;
