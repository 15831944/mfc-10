...
template <class T>
CCollection<T>::CCollection(const char* name, long _limit, long _delta) : Name(name)
{
...
template <class T>
long CChooseCollection<T>::Find(const char* _name) const
{
	for (long i=0L; i<count; i++)
	{
...
template <class T>
void CChooseCollection<T>::SetSelIndex(const char* _name)
{
	long i = Find(_name);
	if (i>=0) Selected = i;
};
...
template <class T>
bool CChooseCollection<T>::ChooseSelected(const char* /*dialogtitle*/, CWnd* parent)
{
...
template <class T>
long* CChooseCollection<T>::MultiSelection(const char* dialogtitle)
{
	return NULL;
};
