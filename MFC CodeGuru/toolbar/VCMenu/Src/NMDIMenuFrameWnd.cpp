// MDIMenuFrameWnd.cpp : implementation file
//
/*
Copyright (C) 1998 Tony Hoyle (tmh@netfusion.co.uk)

Copyright (C) 1998 George H. Stahl (GStahl@VipMail.Com)
	-added m_pFloatingFrameClass = RUNTIME_CLASS(CNMiniDockFrameWnd);
...
CNMDIMenuFrameWnd::CNMDIMenuFrameWnd()
{
}
...
IMPLEMENT_DYNAMIC(CNMDIMenuFrameWnd, CMDIFrameWnd)

BEGIN_MESSAGE_MAP(CNMDIMenuFrameWnd, CMDIFrameWnd)
	//{{AFX_MSG_MAP(CNMDIMenuFrameWnd)
	ON_WM_CREATE()
	ON_WM_MENUCHAR()
	ON_WM_ACTIVATE()
	//}}AFX_MSG_MAP
...
/////////////////////////////////////////////////////////////////////////////
// CNMDIMenuFrameWnd message handlers

int CNMDIMenuFrameWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	{
		TRACE0("CNMDIMenuFrameWnd: Failed to create menu\n");
		return -1;
	}
...
BOOL CNMDIMenuFrameWnd::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext) 
{
	if(!CMDIFrameWnd::OnCreateClient(lpcs, pContext)) return FALSE;
	m_wndMenu.SubclassMDIClient(m_hWndMDIClient);
	return TRUE;
}

LRESULT CNMDIMenuFrameWnd::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu) 
{
...
void CNMDIMenuFrameWnd::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
...
void CNMDIMenuFrameWnd::EnableDocking(DWORD dwDockStyle)
{
...
BOOL CNMDIMenuFrameWnd::LoadFrame(UINT nIDResource, DWORD dwDefaultStyle,
