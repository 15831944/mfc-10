...
BOOL CMDriveDlg::OnInitDialog()
{
	...
	CoCreateInstance(CLSID_Randexe, NULL, CLSCTX_ALL, IID_IRandexe, (void**)&pRandom);
	_ASSERTE(pRandom != NULL);
...
void CMDriveDlg::OnAdvise() 
{
	if (m_nAdviseCnt < m_nMaxAdvises)
	{
		...
		HRESULT hRes = AtlAdvise(pRandom, pDriver->GetUnknown(), IID_IRandexeEvent, &m_arrAdvise[m_nAdviseCnt++]);
		if (FAILED(hRes))
		{
...
BOOL CMDriveDlg::Unadvise()
{
	if (m_nAdviseCnt > 0)
	{
		HRESULT hRes = AtlUnadvise(pRandom, IID_IRandexeEvent, m_arrAdvise[--m_nAdviseCnt]);
