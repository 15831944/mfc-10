...
void CTitleTip::Show(CRect rectTitle, LPCTSTR lpszTitleText, int xoffset /*=0*/)
{
       ...
       // Show the titletip
        ...
        if( IsWindowVisible() ) 
		{
				...
				dc->DrawText(strTitle,-1,rectTitle, DT_LEFT | DT_SINGLELINE | DT_NOPREFIX | DT_NOCLIP | DT_VCENTER);
...
CJCombo::CJCombo()
{
	...
	// comments in JCombo.h explain how to do this for 
	// CJCombo as implemented here.
...
BOOL CJCombo::AddItem(CString strItem,int nCol,int nRow,int nMask,int nFmt)
{
	if (nRow != PrevRow)
		AddString(strItem);
   	PrevRow = nRow;
