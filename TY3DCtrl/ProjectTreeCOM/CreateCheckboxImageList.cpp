...
//
BOOL CreateCheckboxImageList(CDCHandle pDC, 
...
/*1110*/_T("checked disabled"),		CBS_CHECKEDDISABLED,	DFCS_BUTTONCHECK | DFCS_CHECKED | DFCS_INACTIVE | DFCS_FLAT,
	...
	if (pDC && (nSize > 0))
	{
		...
		ATLASSERT(nImages == 17);

 

		WTL::CBitmap bmpCheckboxes;
					...
					nImages, 1))
			{
				...
				if (dcMem.CreateCompatibleDC(pDC))
				{
					...
					hTheme = (IsThemeActive() && 
							  IsAppThemed()) ? 
							  theme.OpenThemeData(NULL, L"BUTTON") : NULL;
					WTL::CBitmapHandle pOldBmp = dcMem.SelectBitmap(bmpCheckboxes);
					...
					WTL::CRect rectImage(nImageLeft, 
									nImageTop, 
					...
					for (int i = 0; cbdd[i].pszDesc != NULL; i++)
					{
						...
						else
						{
							if (hTheme)
							{
								theme.DrawThemeBackground(dcMem, 
									BP_CHECKBOX, cbdd[i].nStateId, &rectImage, 
									NULL);
								theme.DrawThemeEdge(dcMem, 
				...
				{
					ATLTRACE(_T("ERROR - failed to create DC\n"));
				}
			}
			else
			{
				ATLTRACE(_T("ERROR - failed to create image list\n"));
			}
		}
		else
		{
			ATLTRACE(_T("ERROR - failed to create bitmap\n"));
		}
	}
	else
	{
		ATLTRACE(_T("ERROR - bad parameters\n"));
	}
	return rc;
}
