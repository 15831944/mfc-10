				...
				for (int x = 0; x < szTable.cx; x++)
				{
					...
					if (sc.nColSpan == i)
					{
						if (i == 1)
						{
							...
							if( cur_table.width [x] > sc.szCell.cx )
								cur_table.width [x] = cur_table.width [x];
							else
								cur_table.width [x] = sc.szCell.cx;
							if (sc.bFixedWidth)
				...
				for (int x = 0; x < szTable.cx; x++)
				{
					...
					if (sc.nRowSpan == i)
					{
						...
						//	cur_table.height [y] = max (cur_table.height [y], sc.szCell.cy);
							if( cur_table.height [y] > sc.szCell.cy )
								cur_table.height [y] = cur_table.height [y];
							else
								cur_table.height [y] = sc.szCell.cy;
						}
						else
						{
							int span_height = 0;
...
//		sc.szText.cy = m_defStyle.nCellHeight;
	...
	if (MODE_DRAW != m_nNumPass)
	{
		...
		if( m_defStyle.nCellWidth > sc.szText.cx )
			sc.szCell.cx = m_defStyle.nCellWidth;
		else
			sc.szCell.cx = sc.szText.cx;

		//sc.szCell.cy = max(m_defStyle.nCellHeight, sc.szText.cy);
		if( m_defStyle.nCellHeight > sc.szText.cy )
			sc.szCell.cy = m_defStyle.nCellHeight;
		else
			sc.szCell.cy = sc.szText.cy;
...
//	pt.y = lpRect->top;
	...
	for (i = nFirstLine; i < m_nCurLine; i++)
	{
		...
		if (0 != m_hline.nAddPercentWidth)
		{
			...
			szTextArea.cx = (szTextArea.cx> m_hline.nWidthLine) ? szTextArea.cx : m_hline.nWidthLine;
		} //if
	} //for
//
//	if (NULL != lpSize)
...
//		m_hline.nDescentLine = max(m_hline.nDescentLine, nHeight - m_tm.tmAscent);
		m_hline.nDescentLine = (m_hline.nDescentLine > (nHeight - m_tm.tmAscent)) ? m_hline.nDescentLine: (nHeight - m_tm.tmAscent);
//		m_hline.nHeightLine = max(m_hline.nHeightLine, m_tm.tmAscent);
		m_hline.nHeightLine = (m_hline.nHeightLine > m_tm.tmAscent) ? m_hline.nHeightLine: m_tm.tmAscent;
	}
	else if (MODE_DRAW == m_nNumPass)
	{
		switch (m_defStyle.nVertAlign)
...
//	m_nDarkenShadow = min(100, nDarkenPercent);
	m_nDarkenShadow = 100 < nDarkenPercent ? 100 : nDarkenPercent;
