...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CHttpLicense::VerifyLicenseKey(BSTR bstr)   
{
	USES_CONVERSION;
...
Return
	TRUE		- success
****************************************************/
BOOL CHttpLicense::GetLicenseKey(DWORD dwReserved, BSTR* pBstr) 
{  
	USES_CONVERSION;
...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CHttpLicense::IsLicenseValid() 
{
	USES_CONVERSION;
	...
	if( ! m_bLicense ) {
		CAboutDlg	dlg("Can't create Http Control! License key is missing."); 
		dlg.DoModal();
		}
