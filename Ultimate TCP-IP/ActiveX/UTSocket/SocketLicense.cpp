//=================================================================
//  class: CSocketLicense
//  File:  SocketLicense.cpp
//
//  Socket ActiveX control license support
//
//=================================================================
...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CSocketLicense::VerifyLicenseKey(BSTR bstr)   
{
	USES_CONVERSION;
...
Return
	TRUE		- success
****************************************************/
BOOL CSocketLicense::GetLicenseKey(DWORD dwReserved, BSTR* pBstr) 
{  
	USES_CONVERSION;
...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CSocketLicense::IsLicenseValid() 
{
	USES_CONVERSION;
	...
	if( ! m_bLicenseChecked ) {					
	   ...
	   strcat(szLicenseFile, "\\data.lic");

		// Open the file for reading
		FILE	*hFile = fopen( szLicenseFile, "rb" );
	...
	if( ! m_bLicense ) {
		CAboutDlg	dlg(_T("Can't create Socket Control! License key is missing.")); 
		dlg.DoModal();
		}
