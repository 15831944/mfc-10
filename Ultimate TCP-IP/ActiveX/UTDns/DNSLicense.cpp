...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CDNSLicense::VerifyLicenseKey(BSTR bstr)   
{
	USES_CONVERSION;
...
Return
	TRUE		- success
****************************************************/
BOOL CDNSLicense::GetLicenseKey(DWORD dwReserved, BSTR* pBstr) 
{  
	USES_CONVERSION;
...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CDNSLicense::IsLicenseValid() 
{
	USES_CONVERSION;
	...
	if( ! m_bLicense ) {
		CAboutDlg	dlg("Can't create DNS Control! License key is missing."); 
		dlg.DoModal();
		}
