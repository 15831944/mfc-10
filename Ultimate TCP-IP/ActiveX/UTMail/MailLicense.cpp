//=================================================================
//  class: CMailLicense
//  File:  MailLicense.cpp
//
//  Mail ActiveX control license support
//
//=================================================================
...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CMailLicense::VerifyLicenseKey(BSTR bstr)   
{
	USES_CONVERSION;

#ifndef EVALUATION_MODE
	return !lstrcmp(OLE2T(bstr), _T(RUNTIME_LICENSE_KEY));  
#elif defined(_BLOCKING_MODE_ONLY)
    return !lstrcmp(OLE2T(bstr), _T(RUNTIME_LICENSE_KEY));  
#else
	CAboutDlg	dlg; 
	dlg.DoModal();
...
Return
	TRUE		- success
****************************************************/
BOOL CMailLicense::GetLicenseKey(DWORD dwReserved, BSTR* pBstr) 
{  
	USES_CONVERSION;
...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CMailLicense::IsLicenseValid() 
{
	USES_CONVERSION;
	...
	if( ! m_bLicense ) {
		CAboutDlg	dlg("Can't create Mail Control! License key is missing."); 
		dlg.DoModal();
		}
