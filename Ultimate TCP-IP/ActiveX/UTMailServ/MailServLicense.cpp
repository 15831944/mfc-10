...
Return
	none
****************************************************/
CMailServLicense::CMailServLicense() 
{
}
...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CMailServLicense::VerifyLicenseKey(BSTR bstr)   
{
	USES_CONVERSION;
...
Return
	TRUE		- success
****************************************************/
BOOL CMailServLicense::GetLicenseKey(DWORD dwReserved, BSTR* pBstr) 
{  
	USES_CONVERSION;
...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CMailServLicense::IsLicenseValid() 
{
	USES_CONVERSION;
	...
	if( ! m_bLicenseChecked ) {					
	   ...
	   strcat(szLicenseFile, "\\datas.lic");
	...
	if( ! m_bLicense ) {
		CAboutDlg	dlg(_T("Can't create MailServer Control! License key is missing.")); 
		dlg.DoModal();
		}
