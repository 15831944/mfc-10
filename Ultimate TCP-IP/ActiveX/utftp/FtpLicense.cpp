...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CFtpLicense::VerifyLicenseKey(BSTR bstr)   
{
	USES_CONVERSION;
...
Return
	TRUE		- success
****************************************************/
BOOL CFtpLicense::GetLicenseKey(DWORD dwReserved, BSTR* pBstr) 
{  
	USES_CONVERSION;
...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CFtpLicense::IsLicenseValid() 
{
	USES_CONVERSION;
	...
	if( ! m_bLicense ) {
		CAboutDlg	dlg("Can't create Ftp Control! License key is missing."); 
		dlg.DoModal();
		}
