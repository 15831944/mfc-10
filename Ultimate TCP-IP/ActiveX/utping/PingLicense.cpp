...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CPingLicense::VerifyLicenseKey(BSTR bstr)   
{
	USES_CONVERSION;
...
Return
	TRUE		- success
****************************************************/
BOOL CPingLicense::GetLicenseKey(DWORD dwReserved, BSTR* pBstr) 
{  
	USES_CONVERSION;
...
Return
	...
	FALSE	- if not
****************************************************/
BOOL CPingLicense::IsLicenseValid() 
{
	USES_CONVERSION;
	...
	if( ! m_bLicense ) {
		CAboutDlg	dlg("Can't create Ping Control! License key is missing."); 
		dlg.DoModal();
		}
