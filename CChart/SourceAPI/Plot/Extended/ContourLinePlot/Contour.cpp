...
/*	double mn, mx, val;
	int i, j;
	double dx, dy;
	dx = (m_pLimits[1] - m_pLimits[0]) / m_iColSec;
	dy = (m_pLimits[3] - m_pLimits[2]) / m_iRowSec;
	mn = mx = m_pFieldFcn(m_pLimits[0], m_pLimits[2]);
	for (i = 0; i <= m_iColSec; i++)
	{
		for (j = 0; j <= m_iRowSec; j++)
		{
			val = m_pFieldFcn(m_pLimits[0] + i * dx, m_pLimits[2] + j * dy);
			if (val > mx)
				mx = val;
			if (val < mn)
				mn = val;
	...
	for (int i = 0; i < nPlanes; i++)
	{
		if (!bExponential || mn <= 0.0)
		{
			vPlanes[i] = max - i * (max - min) / (nPlanes - 1.0);
		}
		else
		{
			vPlanes[i] = max / pow(pow(max / min, 1.0 / (nPlanes - 1.0)), i);
		}
	}
...
void CContour::SetPlotRange(double pLimits[])
{
