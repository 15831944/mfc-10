...
BOOL CMessageBoxDialog::OnInitDialog ( )
{
	...
	if ( m_nTimeoutSeconds > 0 )
	{
		...
		if ( m_bTimeoutDisabled )
		{
			...
			for ( int i = 0; i < m_aButtons.GetCount(); i++ )
			{
...
void CMessageBoxDialog::ParseStyle ( )
{
	...
	if ( m_nStyle & MB_DEFMASK )
	{
		...
		if ( m_aButtons.GetCount() >= ( nDefaultIndex + 1 ) )
		{
...
void CMessageBoxDialog::CreateButtonControls ( )
{
	...
	m_sButton.SetSize(XDialogUnitToPixel(CX_BUTTON),
...
void CMessageBoxDialog::DefineLayout ( )
{
	...
	int cxButtons =
		( m_aButtons.GetCount() - 1 ) * XDialogUnitToPixel(CX_BUTTON_SPACE) +
		m_aButtons.GetCount() * m_sButton.cx;
