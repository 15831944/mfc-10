	...
	COLORREF clrpTransColor, int iTransPixelX, int iTransPixelY )
{
	...
	if(m_bCustomerSetLength)
	{
		if (m_bVertical)
		{
			GetClientRect( &m_rect );
			SetWindowPos(NULL, 0, 0, m_nWidth, m_rect.Height(), SWP_NOZORDER | SWP_NOMOVE);
			GetClientRect( &m_rect );
		...
		{
			GetClientRect( &m_rect );
			SetWindowPos(NULL, 0, 0, m_rect.Width(), m_nHeight, SWP_NOZORDER | SWP_NOMOVE);
			GetClientRect( &m_rect );
			m_nWidth = m_rect.Width();
			//m_nHeight = m_rect.Height();
		}
	...
	GetClientRect( &m_rect );//here,get the same rec to bitmap
...
BOOL CBitmapSlider::SetBitmapThumb(
	UINT nThumbID, UINT nActiveID,UINT nDisableID,BOOL bTransparent,
	COLORREF clrpTransColor, int iTransPixelX, int iTransPixelY )
{
	...
	if( !nThumbID ) {
		...
		m_bmThumbDisable.DeleteObject();
		m_bmThumbDisableMask.DeleteObject();
	...
	// Load a bitmap for disable state.
	if( nDisableID ) {

		m_bmThumbDisable.DeleteObject();

		if( !m_bmThumbDisable.LoadBitmap( nDisableID ) ) {
		...
		if( bTransparent ) {
			...
			PrepareMask( &m_bmThumbDisable, &m_bmThumbDisableMask,
				clrpTransColor, iTransPixelX, iTransPixelY );
		}

		m_bThumbDisable = TRUE;

		// There is no bitmap for disable state.
	} else
		m_bThumbDisable = FALSE;
...
// 			pTmpDC, xSource, ySource, SRCCOPY );
		//////////////////////////////////////////////////////////////////////////
		BITMAP	bitmap;
		pTmpDC->GetCurrentBitmap()->GetBitmap( &bitmap );
		if (wWidth > bitmap.bmWidth - xSource)//如果要绘图的部分比原bitmap宽
		{
			int curWidth,paintWidth;
			for (curWidth = xStart,paintWidth = 0; curWidth < wWidth + xStart; curWidth += paintWidth)
			{
				paintWidth = min(bitmap.bmWidth - (curWidth % bitmap.bmWidth),wWidth  + xStart - curWidth);
				if(paintWidth <= 0)
					return;
				pDC->BitBlt( curWidth, yStart,
					paintWidth, wHeight,
					pTmpDC, curWidth % bitmap.bmWidth, ySource, SRCCOPY );
			}
		}
		else
		{
