...
BOOL CDSBuffer::Create(IDirectSound8 *pDirectSound, const CWave &wave, DWORD dwFlags)
{
	...
	if( FAILED( hr = lpDSB->QueryInterface(IID_IDirectSoundBuffer8, (LPVOID *) &m_pDSBuffer)))
	{
...
BOOL CDSBuffer::Create(IDirectSound8 * pDirectSound, UINT uIDRcsrWave, DWORD dwFlags, HMODULE hMod /*= AfxGetInstanceHandle()*/)
{
	CWave wave ;
...
BOOL CDSBuffer::Create(IDirectSound8 * pDirectSound, LPCTSTR lpszFileName, DWORD dwFlags)
{
	CWave wave;
