...
const
  CI_ReadTimeOut = 15 * 1000; //默认超时15S
type  
  {* 基础协议对象 *}
...
begin
  try
    ...
    try
      ...
      FClient.ReadCardinal(False); //读取命令长度
...
var
  dwPacketLen, dwCode: Cardinal;
  utf8Buff: UTF8String;
begin
  try
    FClient.ReadTimeout := CI_ReadTimeOut;
    FClient.MaxLineLength := 1024 * 1024 * 256;
    ...
    AResponse.Clear;
    AResponse.Text := Utf8ToAnsi(utf8Buff);
    dwCode := Cardinal(StrToInt(AResponse.Values[CSCode]));
    Result := dwCode = 0;
    if not Result then
      FLastError := GetErrorCodeString(dwCode) + ': ' + AResponse.Values[CSMessage];
  except
    on E: Exception do
    begin
      FLastError := E.Message;
...
function TThreadClientSocket.RecvData(var ALen: Integer): Boolean;
var
