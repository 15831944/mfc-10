...
uses
  ...
  CommCtrl, VirtualTrees, XMLDoc, XMLIntf, Math, Dialogs, Variants;
...
begin
  ...
  StringReplace(SourXml, #$9, '', [rfReplaceAll]);
  StringReplace(SourXml, #$D, '', [rfReplaceAll]);
  StringReplace(SourXml, #$A, '', [rfReplaceAll]);
  //DoImportXml(Title, EDoc);
  Result := '';
